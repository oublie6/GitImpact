...
- **默认完全禁用 GORM AutoMigrate**（`database.auto_migrate: false`）。
- GitImpact 启动时不会自动建表或改表，避免达梦兼容性问题（如 `longtext` 类型、重复 `ALTER TABLE ADD COLUMN`）。
- 必须先执行手工 SQL 初始化：
  - MySQL：`sql/mysql/init.sql`
  - 达梦：`sql/dameng/init.sql`
- 如果数据库未初始化，服务会在启动阶段给出清晰报错（缺失核心表 + 应执行的 SQL 路径）。
